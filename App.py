...
def movie_poster_fetcher(imdb_link):
    # print(imdb_link , 'dbajdbj')
    # ## Display Movie Poster
    # url_data = requests.get(imdb_link).text
    # s_data = BeautifulSoup(url_data, 'html.parser')
    # img = s_data.find_all("div", class_="ipc-media")
    # print(s_data[0][1],"hello world")
    ...
    id, = re.findall("tt\d\d\d\d\d\d\d", imdb_link)
    image_link = f"https://img.omdbapi.com/?apikey=a77e1697&i={id}"
    st.image(image_link, use_column_width=False)

def get_movie_info(imdb_link):
    # print(imdb_link , 'dnja')
    # url_data = requests.get(imdb_link).text
    # s_data = BeautifulSoup(url_data, 'html.parser')
...
def run():
    ...
    genres = ['Action', 'Adventure', 'Animation', 'Biography', 'Comedy', 'Crime', 'Documentary', 'Drama', 'Family', 'Fantasy', 'Film-Noir', 'Game-Show', 'History', 'Horror', 'Music', 'Musical', 'Mystery', 'News',
